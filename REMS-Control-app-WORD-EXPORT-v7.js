...
const save=async()=>{
  ...
  if(!cloudReady||!cloudDb){
    setStatus("v5.7 · немає з’єднання");
    return false;
  }
  try{
    cloudWriting=true;
    setStatus("v5.7 · збереження…");
...
// ===== Розклад занять · REMS Control v5.7 =====
...
async function recoverStudentsFromFirebase(){
  ...
  setStatus("v5.7 · аналіз відновлення…");
  ...
  try{
    ...
    setStatus("v5.7 · помилка відновлення");
...
const wordProjectSections=(p,options,allAcks=[])=>{
  ...
  if(options.conflicts){
    ...
    team.forEach(st=>conflictGroupsForStudent(st.id).filter(g=>dateSet.has(g.date)).forEach(g=>{
      const acts=g.events.map(a=>`${activityTitle(a)} (${activityMeta(a)||"час не вказано"})`).join("; ");
        ...
        ${dayItems.map(x=>{
          ...
          if(a.source==="lesson"){
            return `<div class="day-event-row ${activeProjectId!=="__all__"?"day-event-dimmed":""}">
              ...
              <div><b>🎓 ${esc(a.title)}</b><div class="day-event-meta">${esc(a.lessonType||"Заняття")} · ${esc(eventTimeText(a)||"Час не вказано")}${a.location?` · ауд. ${esc(a.location)}`:""} · ${visibleCount} студентів${activeStudentGroup!=="__all__"?` (${esc(activeStudentGroup)})`:""}</div></div>
...
async function initCloud(){
  ...
  if(!cfg){
    setStatus("v5.7 · Firebase не налаштовано");
    dashboard();
  ...
  try{
    setStatus("v5.7 · завантаження хмари…");
...
functions=getFunctions(firebaseApp,"europe-west1");
    ...
    onSnapshot(ref,s=>{
      ...
      cloudReady=false;
      setWriteUiReady(false);
      setStatus("v5.7 · хмара недоступна");
    });
  ...
  try{
    ...
    onAuthStateChanged(auth,async user=>{
      ...
      if(currentUser){
        ...
        clearLogout();
        showLogin();
        setStatus("v5.7 · потрібен вхід");
      }
    });
  }catch(err){
    console.error(err);
    setStatus("v5.7 · помилка авторизації");
    showLogin();
  }
}
