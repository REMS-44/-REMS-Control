...
function academicV39(){
  ...
  const render=()=>{
    ...
    const card=(items,shared=false)=>{const l=items[0];const isIndividual=String(l.lessonType||"").toLowerCase().includes("індив");const exactTime=(isIndividual&&l.startTime&&l.endTime)?`<div class="academic-v393-exact-time">${esc(l.startTime)}–${esc(l.endTime)}</div>`:"";return `<button type="button" class="academic-dual-card academic-v393-card ${isIndividual?"academic-v405-individual":""} ${academicTeacherClass(l.teacher,l.subject)} ${shared?"shared":""}" data-ids="${esc(items.map(x=>String(x.id)).join(","))}" data-date="${esc(l.date||"")}">${shared?'<div class="academic-v393-together">РАЗОМ · РЕМС-34 + РЕМС-44</div>':""}${exactTime}<div class="academic-v393-subject">${esc(l.subject||"Заняття")}</div><div class="academic-v393-kind">${esc(academicDisplayLessonType(l.lessonType))}</div><div class="academic-v393-teacher">${esc(l.teacher||"Викладача не вказано")}</div>${String(l.scope||"")==="selected"?`<div class="academic-v393-students">👤 ${esc(lessonStudents(l).map(s=>s.name).join(", ")||String(l.note||"").replace(/^Індивідуальне заняття ·\s*/,""))}</div>`:""}<div class="academic-v393-room">ауд. ${esc(String(l.room||"").trim()||"не вказана")}</div></button>`;};
