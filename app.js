...
const save=async()=>{
  ...
  if(!cloudReady||!cloudDb){
    setStatus("v1.7 · немає з’єднання");
    return false;
  }
  try{
    cloudWriting=true;
    setStatus("v1.7 · збереження…");
        ...
        <div class="today-events">
          ${todayEvents.map(x=>`<div class="today-event">
            ...
            <div>
              ...
              <small>${esc(x.event.type)}${eventMetaText(x.event)?` · ${esc(eventMetaText(x.event))}`:""} · ${x.students.length} студентів</small>
            </div>
...
function openStudent(id){
  ...
  const renderStudentMonth=(month)=>{
    ...
    const cells=Array.from({length:last.getDate()},(_,i)=>{
      ...
      return `<div class="student-month-day ${dow===0||dow===6?"weekend":""}">
        ...
        <div class="student-day-events">
          ${dayItems.map((x,idx)=>`<div class="student-day-event" data-date="${date}" data-index="${idx}" style="background:${x.p.color}" title="${esc(x.p.name)} · ${esc(x.type)}${eventMetaText(x)?` · ${esc(eventMetaText(x))}`:""}">${eventTimeText(x)?`${esc(eventTimeText(x))} · `:""}${shortType(x.type)}</div>`).join("")}
        </div>
      </div>`;
    }).join("");
    ...
    $("#studentCalendarView").querySelectorAll(".student-day-event").forEach(el=>el.onclick=()=>{
      ...
      const dayItems=items.filter(x=>x.date===date);
      const item=dayItems[idx]||dayItems[0];
  ...
  dialog.querySelector("#projectCardBody").innerHTML=`<div class="project-body">
    ...
    <div class="day-event-list">
      ${evs.map((e,i)=>`<div class="day-event-row">
        ...
        <div><b>${esc(e.type)}</b>${eventMetaText(e)?`<div class="day-event-meta">${esc(eventMetaText(e))}</div>`:""}<div class="day-event-meta">${studentsForEvent(e).length} учасників</div></div>
...
function editProjectEvent(projectId,ev){
  ...
  dialog.querySelector("#projectCardBody").innerHTML=`<div class="project-body">
    ...
    <form id="projectEventEditForm" class="project-edit-form" style="margin-top:16px">
      ...
      <label>Початок<input id="editEventStartTime" type="time" value="${esc(ev.startTime||"")}"></label>
      <label>Завершення<input id="editEventEndTime" type="time" value="${esc(ev.endTime||"")}"></label>
      <label class="full">Локація<input id="editEventLocation" value="${esc(ev.location||"")}" placeholder="Напр. ВДНГ · павільйон 3 / ауд. 230 / студія"></label>
      <div class="full profile-actions">
...
async function initCloud(){
  ...
  if(!cfg){
    setStatus("v1.7 · Firebase не налаштовано");
    dashboard();
  ...
  try{
    setStatus("v1.7 · завантаження хмари…");
    ...
    onSnapshot(ref,s=>{
      ...
      cloudReady=false;
      setWriteUiReady(false);
      setStatus("v1.7 · хмара недоступна");
    });
  ...
  try{
    ...
    onAuthStateChanged(auth,async user=>{
      ...
      if(currentUser){
        ...
        clearLogout();
        showLogin();
        setStatus("v1.7 · потрібен вхід");
      }
    });
  }catch(err){
    console.error(err);
    setStatus("v1.7 · помилка авторизації");
    showLogin();
  }
}
