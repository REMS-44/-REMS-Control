...
function projects(){
  ...
  app.innerHTML=`<div class="projects-grid-main">${orderedProjects.map(p=>{
...
function showDay(date){
  ...
  const render=()=>{
    ...
    const activeStudentIds=activeGroup?.studentIds||occupiedIds;
    const activeStudents=(db.students||[]).filter(st=>activeStudentIds.has(String(st.id)));
    const busyElsewhere=activeGroup
      ? (db.students||[]).filter(st=>occupiedIds.has(String(st.id))&&!activeGroup.studentIds.has(String(st.id)))
      : [];
    ...
    dialog.querySelector("#dayDialogBody").innerHTML=`<div class="day-panel">
      ...
      ${groups.length?`<div class="day-project-switcher" aria-label="Перемикання між проєктами">
        <button type="button" class="day-project-tab ${activeProjectId==="__all__"?"active":""}" data-day-project="__all__">Усі · ${occupiedIds.size}</button>
        ${groups.map(g=>`<button type="button" class="day-project-tab ${String(activeProjectId)===String(g.project?.id)?"active":""}" data-day-project="${esc(String(g.project?.id||""))}" style="--day-project-color:${g.project?.color||"#6b7280"}">${projectLogoHtml(g.project,"day-project-tab-logo")}<span>${esc(g.project?.name||"Проєкт")}</span><b>${g.studentIds.size}</b></button>`).join("")}
      </div>`:""}
      ...
      ${activeGroup?`<div class="day-project-focus-head" style="--day-project-color:${activeGroup.project?.color||"#6b7280"}">
        <div>${projectLogoHtml(activeGroup.project,"day-project-focus-logo")}<span><small>ВИБРАНИЙ ПРОЄКТ</small><b>${esc(activeGroup.project?.name||"Проєкт")}</b><em>${activeGroup.events.map(e=>esc(e.type||"Подія")).join(" · ")}</em></span></div>
        <div class="muted">${busyElsewhere.length} зайняті в інших проєктах або на заняттях</div>
      </div>`:""}

      <div class="availability-grid-two">
        <div class="availability-card">
          <b>${activeGroup?`У ПРОЄКТІ · ${activeStudents.length}`:`ЗАЙНЯТІ · ${occupiedIds.size}`}</b>
          <div class="availability-list">
            ${activeStudents.map(st=>`<button class="availability-chip day-student" data-id="${esc(String(st.id))}">${esc(st.name)}</button>`).join("")||`<span class="muted">${activeGroup?"У цьому проєкті цього дня ще нікого немає.":"Зайнятих студентів немає."}</span>`}
          </div>
        </div>
        <div class="availability-card">
          <div class="availability-card-head">
            <b>${activeGroup?`ВІЛЬНІ ЦЬОГО ДНЯ · ${freeStudents.length}`:`ВІЛЬНІ · ${freeStudents.length}`}</b>
          ...
          <div class="availability-list ${activeGroup?"day-free-list":""}">
            ${freeStudents.map(st=> activeGroup
              ? `<span class="day-free-person ${pendingAddIds.has(String(st.id))?"selected":""}"><button class="availability-chip day-student" data-id="${esc(String(st.id))}">${esc(st.name)}</button><button type="button" class="day-add-person ${pendingAddIds.has(String(st.id))?"selected":""}" data-add-id="${esc(String(st.id))}" title="${pendingAddIds.has(String(st.id))?"Прибрати з вибраних":"Додати до вибраних"}">${pendingAddIds.has(String(st.id))?"✓":"+"}</button></span>`
              : `<button class="availability-chip day-student" data-id="${esc(String(st.id))}">${esc(st.name)}</button>`
            ).join("")||'<span class="muted">Вільних студентів немає.</span>'}
          </div>
...
function openAcademicBulkEditor(keys=[]){
  ...
  const byKey={
    subject:["#bulkSubject"],lessonType:["#bulkLessonType"],time:["#bulkStartTime","#bulkEndTime"],
  ...
  if(studentPick){
    studentPick.innerHTML=`<div class="academic-student-pick-head"><b>Оберіть студентів</b><span>${students.length} у групі</span></div>
      <div class="academic-student-grid">${students.map(st=>`<label class="academic-student-check"><input type="checkbox" value="${esc(String(st.id))}" disabled><span>${esc(st.name)}</span></label>`).join("")||'<span class="muted">У цій групі студентів немає.</span>'}</div>`;
  }
  ...
  form.onsubmit=async e=>{
    ...
    const start=body.querySelector("#bulkStartTime")?.value||"";
    const end=body.querySelector("#bulkEndTime")?.value||"";
    if(enabled.has("time")&&start&&end&&timeMinutes(start)>=timeMinutes(end)){alert("Час завершення має бути пізніше за час початку.");return;}
    let studentIds=[];
    ...
    for(const item of occurrences){
      ...
      if(enabled.has("time")){target.startTime=start;target.endTime=end;}
