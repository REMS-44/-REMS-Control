...
const save=async()=>{
  ...
  if(!cloudReady||!cloudDb){
    setStatus("v5.8 · немає з’єднання");
    return false;
  }
  try{
    cloudWriting=true;
    setStatus("v5.8 · збереження…");
...
async function recoverStudentsFromFirebase(){
  ...
  setStatus("v5.8 · аналіз відновлення…");
  ...
  try{
    ...
    setStatus("v5.8 · помилка відновлення");
...
async function initCloud(){
  ...
  if(!cfg){
    setStatus("v5.8 · Firebase не налаштовано");
    dashboard();
  ...
  try{
    setStatus("v5.8 · завантаження хмари…");
...
functions=getFunctions(firebaseApp,"europe-west1");
    ...
    onSnapshot(ref,s=>{
      ...
      cloudReady=false;
      setWriteUiReady(false);
      setStatus("v5.8 · хмара недоступна");
    });
  ...
  try{
    ...
    onAuthStateChanged(auth,async user=>{
      ...
      if(currentUser){
        ...
        clearLogout();
        showLogin();
        setStatus("v5.8 · потрібен вхід");
      }
    });
  }catch(err){
    console.error(err);
    setStatus("v5.8 · помилка авторизації");
    showLogin();
  }
}
