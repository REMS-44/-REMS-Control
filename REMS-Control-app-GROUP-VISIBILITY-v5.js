    ...
    dialog.querySelector("#dayDialogBody").innerHTML=`<div class="day-panel">
      ...
      <div class="availability-grid-two">
        <div class="availability-card">
          <div class="availability-title"><b>${activeGroup?`У ПРОЄКТІ · ${activeStudents.length}`:`ЗАЙНЯТІ · ${occupiedIds.size}`}</b><small>${esc(studentGroupSummary(activeStudents)||"—")}</small></div>
          <div class="availability-list">
            ${activeStudents.map(st=>`<button class="availability-chip day-student" data-id="${esc(String(st.id))}">${studentIdentityHtml(st,studentBusyLabelsOnDate(st.id,date).join(" · "))}</button>`).join("")||`<span class="muted">${activeGroup?"У цьому проєкті цього дня ще нікого немає.":"Зайнятих студентів немає."}</span>`}
          </div>
        </div>
        <div class="availability-card">
          <div class="availability-card-head">
            <div class="availability-title"><b>${activeGroup?`ВІЛЬНІ ЦЬОГО ДНЯ · ${freeStudents.length}`:`ВІЛЬНІ · ${freeStudents.length}`}</b><small>${esc(studentGroupSummary(freeStudents)||"—")}</small></div>
      ...
      ${activeGroup&&busyElsewhere.length?`<div class="availability-card day-busy-elsewhere"><div class="availability-title"><b>ЗАЙНЯТІ В ІНШОМУ · ${busyElsewhere.length}</b><small>${esc(studentGroupSummary(busyElsewhere)||"—")}</small></div><div class="availability-list">${busyElsewhere.map(st=>`<button class="availability-chip day-student" data-id="${esc(String(st.id))}">${studentIdentityHtml(st,studentBusyLabelsOnDate(st.id,date).join(" · "))}</button>`).join("")}</div></div>`:""}
    </div>`;
